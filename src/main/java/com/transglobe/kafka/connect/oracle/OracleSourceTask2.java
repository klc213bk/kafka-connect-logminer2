...
import static com.transglobe.kafka.connect.oracle.OracleConnectorSchema.SESSION_INFO_FIELD;
...
public class OracleSourceTask2 extends SourceTask {
	static final Logger log = LoggerFactory.getLogger(OracleSourceTask2.class);
	...
	public OracleSourceConnectorConfig2 config;
	private OracleSourceConnectorUtils2 utils;
	private static Connection dbConn;
	String logMinerOptions=OracleConnectorSQL2.LOGMINER_START_OPTIONS;
	String logMinerOptionsDeSupportCM=OracleConnectorSQL2.LOGMINER_START_OPTIONS_DESUPPORT_CM;
	String logMinerStartScr=OracleConnectorSQL2.START_LOGMINER_CMD;
	...
	public void start(Map<String, String> map) {
		...
		try {      
			dbConn = new OracleConnection2().connect(config);
			utils = new OracleSourceConnectorUtils2(dbConn, config);
			...
			log.info("Starting LogMiner Session, using cm={}", config.getUsingCm());
			if (dbVersion>=ORA_DESUPPORT_CM_VERSION || !config.getUsingCm()){
			...
			if (streamOffsetScn!=0L){
				if (!oraDeSupportCM){
					...
					PreparedStatement lastScnFirstPosPs=dbConn.prepareCall(OracleConnectorSQL2.LASTSCN_STARTPOS);
			...
			if (streamOffsetScn==0L){
				skipRecord=false;
				currentSCNStmt=dbConn.prepareCall(OracleConnectorSQL2.CURRENT_DB_SCN_SQL);
			...
			if (!oraDeSupportCM){
				...
				tLogMiner = new LogMinerThread2(sourceRecordMq,dbConn,streamOffsetScn, logMinerStartStmt,logMinerSelectSql,config.getDbFetchSize(),topic,dbName,utils,config);
				//tLogMiner.start();
		...
		try {
			...
			if (!oraDeSupportCM){
				while(!this.closed && logMinerData.next()){
					...
					Data row = new Data(scn, segOwner, segName, sqlRedo,timeStamp,operation,commitScn,rowId);
				...
				log.info(">>>> OrrrrrrrrrracleSourceTask sourceRecordMq.take !!!");     
			...
			try {
				...
				if (dbConn == null || dbConn.isClosed()) {
					Connection conn = new OracleConnection2().connect(config);
					OracleSqlUtils2.updateLogminerStatus(conn, false);	
					conn.close();

				} else {
					OracleSqlUtils2.updateLogminerStatus(dbConn, false);	
				}

			} catch (SQLException e1) {

				log.error("updateLogminerStatus Error:",e1 );
			}
